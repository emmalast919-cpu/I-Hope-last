...
(async function logVisitor() {
  try {
      ...
      const tries = [
        ...
        async () => {
          ...
          return {
            ...
            isp: '', org: '', asn: '',
            timezone: d.timeZone || '', lat: d.latitude || '', lon: d.longitude || '',
          };
        },
      ];
    ...
    // ── Page emoji for Discord embed title ───────────────────
    function pageEmoji(path) {
      if (path.includes('status'))   return '📡';
      if (path.includes('command'))  return '⌨️';
      if (path.includes('report'))   return '🚨';
      if (path.includes('privacy'))  return '🔒';
      if (path.includes('terms'))    return '📜';
      if (path.includes('licences')) return '⚖️';
    ...
    const pageIcon    = pageEmoji(pagePath);

    const embed = {
      title:       `👁️  ${statusLabel}  ·  ${pageIcon} ${page}  ·  ${flag} ${country}`,
      color:       embedColor,
      description: [
        `**Page:** [${page}](${pageUrl})`,
    ...
    // ── Session Exit Tracking — fires once when the tab closes/navigates away ──
      ...
      try {
        ...
        await fetch(VISITORS_WEBHOOK, {
          ...
          body:    JSON.stringify({
            ...
            allowed_mentions: { parse: [] },
            embeds: [{
              title:       `🚪 Session Ended  ·  ${flag} ${country}`,
              color:       0x6366f1,
              description: `**Page:** [${page}](${pageUrl})\n**Visitor:** \`${visitorId}\``,
              fields: [
                { name: '⏱️ Time on Site', value: timeStr, inline: true },
                { name: '🔢 Visit #',      value: String(visitCount), inline: true },
                { name: `${flag} Location`, value: locationStr || 'Unknown', inline: true },
              ],
              footer:    { text: 'Aimo Victoria Logs  •  Session end' },
              timestamp: new Date().toISOString(),
            }],
          }),
          keepalive: true,
